...
describe('Home Behavior', () => {
  ...
  it('should show at least 2 projects in the page', () => {
    // Act.
    cy.getByTestId('filterProjects').click().contains('active', { matchCase: false }).click();
    // Assert.
  ...
  it('should show 8 projects when filtering by all', () => {
    // Act.
    cy.getByTestId('filterProjects').click().contains('all', { matchCase: false }).click();
  ...
  it('should show only archived projects when filtering by archived', () => {
    // Act.
    cy.getByTestId('filterProjects').click().contains('archived', { matchCase: false }).click();

    cy.getByTestId('project').its('length').as('projectsCount');
    cy.getByTestId('projectArchived').its('length').as('archivedProjectsCount');

    // Assert.
    cy.get('@projectsCount').then((projectsCount) => cy.get('@archivedProjectsCount').should('eq', projectsCount));
  });
  ...
  it('should show the projects ordered from A to Z', () => {
    ...
    // Act.
    cy.get('[data-testid="sortProjects"] > div').click().contains('from a to z', { matchCase: false }).click();
