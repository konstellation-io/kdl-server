import { AnimatedAreaSeries, AnimatedAxis, AnimatedGrid, XYChart } from '@visx/xychart';
...
function TimeSeriesChart({
  ...
  const { showTooltip, tooltipOpen, hideTooltip, tooltipData, tooltipTop = 0, tooltipLeft = 0 } = useTooltip<D>();
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const xyChartYScale: any = useMemo(() => ({ type: 'linear', domain: yScale.domain() }), [yScale]);
  ...
  return (
    <div className={styles.container}>
      <Legend
        ...
        color={color}
      />
      <XYChart width={width} height={height} xScale={xyChartXScale} yScale={xyChartYScale} margin={margin}>
        <AnimatedGrid columns={false} numTicks={4} lineStyle={gridLineProps} animationTrajectory="max" />
        <AnimatedAxis
          orientation="right"
          left={0}
          numTicks={4}
        ...
        {tooltipData && (
          <g>
            <Line
              ...
              pointerEvents="none"
            />
            <circle cx={tooltipLeft} cy={tooltipTop} r={5} fill={color} strokeWidth={2} pointerEvents="none" />
          </g>
        )}
      </svg>
      <div className={styles.tooltip}>
        <div className={cx(styles.tooltipWrapper, { [styles.open]: tooltipOpen })}>
          {tooltipOpen && tooltipData && (
            ...
            >
              {(props: { tooltipTop: number; tooltipLeft: number }) => (
                <TooltipWithBounds
                  key={Math.random()}
