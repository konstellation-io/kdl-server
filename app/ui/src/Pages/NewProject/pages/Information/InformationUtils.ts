...
export function validateProjectName(name: string, projectsNames: string[] = []) {
  return CHECK.getValidationError([
    CHECK.isFieldNotEmpty(name),
    CHECK.isLengthAllowed(name, maxProjectNameLength),
    CHECK.isItemDuplicated(name, projectsNames, 'name. Please change it and choose a unique project name.'),
  ]);
}
...
export function validateProjectId(id: string, projectsIds: string[] = []) {
  return CHECK.getValidationError([
    ...
    CHECK.isAlphanumeric(replaceAll(id, /-/, ''), 'The id only can contain lowercase alphanumeric and hyphens'),
    CHECK.matches(id, /^[a-z]([-a-z0-9]*[a-z0-9])?$/, 'Enter a valid id consisting of letters, numbers or hyphens.'),
