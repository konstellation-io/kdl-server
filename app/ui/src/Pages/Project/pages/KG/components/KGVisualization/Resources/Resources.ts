...
export default class Resources {
  ...
  init = (container: Selection<SVGGElement, unknown, null, undefined>, data: GroupD[]) => {
    this.data = data;
  ...
  performUpdate = (data: GroupD[]) => {
    ...
    container.selectAll('.removing')
      .interrupt().transition();
    container.selectAll(`.${styles.resourceG}`).transition()
      .delay(100)
  ...
  bindData = (data: GroupD[]) => {
    ...
    return container.select(`.${styles.resourcesWrapper}`)
  ...
  update = (selection: Selection<BaseType, GroupD, BaseType, unknown>) => {
    ...
    selection.select(`.${styles.resource}`)
      .attr('r', resourceR)
      .attr('stroke-width', resourceStroke);
  
    selection.select(`.${styles.resourceImage}`)
      .attr('x', - resourceR / 2)
      .attr('y', - resourceR / 2)
      .attr('width', resourceR)
      .attr('height', resourceR);
    
    selection.select(`.${styles.resourceLabel}`)
      .style('font-size', px(fontSize));
  
    selection.select(`.${styles.tooltipRect}`)
  ...
  highlightResource = (resourceName: string | null) => {
    ...
    const allResources = container.selectAll<BaseType, GroupD>(`.${styles.resource}`);
    allResources
